...
var emptyerror=document.getElementById("emptyerror")
 ...
 if(mail =="" ||!/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(mail)){
   error.email=true
    paraemail.innerText=" Email must be  a valid address.e.g. abc@abc.com"
 ...
 if(password =="" || !/(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}/.test(password)){
   ...
   error.pass=true
    ...
    parapass.innerText= "Password should contain atleast one number and one special character and length must be 6"
